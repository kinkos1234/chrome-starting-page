...
- 실시간 월간 달력, D-day 카운터, 날짜별 할 일(Todo) 관리 지원
- 일상적인 텍스트 기록을 위한 메모장 지원
- 구글, 깃허브, 노션 등 즐겨찾기 사이트의 파비콘(아이콘) 자동 연계 설정
- 사용자 개인 설정과 코어 템플릿의 분리로 깃허브(Github) 등 버전 관리 시 안전한 데이터 보호
- 모든 모달 창은 ESC 키, 배경 클릭으로 빠르게 닫을 수 있는 편의 인터랙션 지원
...
### 2. 달력(Calendar), 투두(Todo), D-day 연동 기능

- **월 변경 내비게이션**: 캘린더 우측 상단에 표시된 현재 월 이름 양옆의 화살표 (`<`, `>`)를 클릭하시면 이전 달이나 다음 달로 날짜를 자유롭게 탐색할 수 있습니다. 다른 달을 보고 있을 때 나타나는 `Today` 버튼으로 현재 월로 즉시 복귀할 수 있습니다.
...
- **D-day 카운터**: 캘린더 헤더의 `+` 버튼을 클릭하면 D-day 관리 모달이 열립니다. 날짜와 제목을 입력하여 중요한 일정(프로젝트 마감, 출장, 연차 등)을 등록하면, 캘린더 그리드 하단에 남은 일수(D-7, D-Day, D+3 등)와 함께 목록이 표시됩니다. D-day가 등록된 날짜는 캘린더 셀에 민트색 테두리로 강조됩니다.
...
이 프로젝트를 실행하려면 **Node.js**가 필요합니다. 아래 링크에서 LTS(장기 지원) 버전을 다운로드하여 설치해주세요. 설치 시 모든 옵션은 기본값 그대로 진행하시면 됩니다.

> **다운로드**: [https://nodejs.org/ko/download/prebuilt-installer](https://nodejs.org/ko/download/prebuilt-installer)

> Node.js를 설치하지 않고 아래 setup 스크립트를 실행하면, 설치 페이지가 자동으로 브라우저에 열립니다.
...
setup 스크립트 실행 후 `http://localhost:1111` 에 접속이 안 되는 경우 아래 사항을 확인해주세요.

| 증상 | 해결 방법 |
|---|---|
| Node.js 설치 직후인데 서버가 안 뜸 | PC를 재부팅한 후 setup 스크립트를 다시 실행해주세요 |
| setup은 성공이라 하는데 접속 안 됨 | (Windows) `start_server.bat`을 더블클릭하여 에러 메시지를 확인해주세요 |
| setup은 성공이라 하는데 접속 안 됨 | (macOS) 터미널에서 `node server.js`를 직접 실행하여 에러 메시지를 확인해주세요 |
| 백신/보안 프로그램 차단 | Windows Defender 등에서 프로젝트 폴더를 예외로 추가해주세요 |
